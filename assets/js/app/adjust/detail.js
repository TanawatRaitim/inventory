		...
		$("#btn_save").on('click',function(e){
			...
			$("input#QTY_Good, input#QTY_Waste, input#QTY_Damage").each(function(){
				...
				if(val=="" || $.isNumeric(val) == false){
					result = false;
					err_description = 'ห้ามใส่ค่าว่าง หรือค่าอื่นที่ไม่ใช่ตัวเลข';
			...
			$("td#total").each(function(){
				...
				if(checked == false && total==0){	
					result = false;
					err_description = 'ห้ามใส่จำนวนรวมเท่ากับ 0';
					return false;
				}
			});
			...
			if(result===true)
			{
				...
				$.ajax({
					...
					success: function(data){
						...
						{
							result = false;
							err_description = data.description;
						}
					},
					beforeSend: function(){
			...
			if(result)
			{
				...
				$("#message").noty({
						//text: "คุณไม่สามารถ่ใส่ค่าว่าง, จำนวนรวมเท่ากับ 0  หรือค่าอื่นที่ไม่ใช่ตัวเลขwได้",
						text: err_description,
						type: 'error',
						dismissQueue: true,
						//killer: true,
						timeout: 10000
