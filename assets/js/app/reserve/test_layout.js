					...
					$.ajax({
						...
						success: function(data){
							...
							if(data.status==true)
							{
								...
								var row_data = '<tr title="'+data.Product_Name+'"><td id="product_no"></td><td>'+product_name+'</td><td class="text-left">';

								row_data += data.Product_Name+'</td><td class="text-center">';
								...
								$("#record_saved > tbody").prepend(row_data);
								...
								//alert(rows);
								$("#product_no").html(rows);
								$("#product_no").each(function(key, value){
									alert(key);
									alert(value);
								});
		...
		$("#delete_confirm").noty({
			...
			buttons     : [
				...
				{addClass: 'btn btn-danger', text: 'Cancel', onClick: function ($noty) {
					...
					$noty.close();
					$("#delete_confirm").noty({
						text:'ยกเลิกลบรายการ ' + product + 'เรียบร้อยแล้ว',
						type:'success',
						timeout: 4000,
						callback:{
							onShow: function(){
